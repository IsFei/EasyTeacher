...
import com.magicare.mutils.common.ParameterizedTypeUtil;
import com.magicare.mutils.ex.HttpException;
import com.magicare.mutils.http.app.HttpRetryHandler;
import com.magicare.mutils.http.app.InterceptRequestListener;
...
import com.magicare.mutils.http.request.UriRequestFactory;
import com.magicare.mutils.task.AbsTask;
import com.magicare.mutils.task.Priority;
import com.magicare.mutils.task.PriorityExecutor;
...
public class HttpTask<ResultType> extends AbsTask<ResultType> implements ProgressHandler {
    ...
    protected ResultType doBackground() throws Throwable {
        ...
        if (cacheCallback != null && HttpMethod.permitsCache(params.getMethod())) {
            ...
            try {
                clearRawResult();
                MLog.d("load cache: " + this.request.getRequestUri());
                rawResult = this.request.loadResultFromCache();
            } catch (Throwable ex) {
                MLog.w("load disk cache error", ex);
            }
            ...
            if (rawResult != null) {
                if (prepareCallback != null) {
                    try {
                        ...
                        cacheResult = null;
                        MLog.w("prepare disk cache error", ex);
                    } finally {
                        clearRawResult();
                    }
                } else {
        ...
        while (retry) {
            ...
            try {
                ...
                try {
                    clearRawResult();
                    // 开始请求工作
                    MLog.d("load: " + this.request.getRequestUri());
                        ...
                        if (redirectHandler != null) {
                            try {
                                ...
                                if (redirectParams != null) {
                                    ...
                                    this.result = M.task().startSync(task);
                                    this.ex = null;
                                }
